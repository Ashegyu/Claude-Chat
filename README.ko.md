...
`/subagent [이름] [프롬프트]` 또는 `/auto-agent [작업설명]`으로 병렬 에이전트를 생성하여 작업을 분배할 수 있습니다. 서브에이전트 활동은 전용 사이드 패널에서 실시간 진행 상황과 함께 표시됩니다.

### 설정

사이드바의 **설정** 버튼 또는 `/settings` 명령어로 열 수 있습니다.

- **일반**: UI 언어(한국어/영어), 모델, Effort Level, Thinking, Fast Mode
- **권한**: Allow/Deny/Ask 규칙
- **플러그인**: 플러그인 활성화/비활성화
- **Hooks**: Pre/PostToolUse, Stop, SessionStart 등
- **고급**: 환경 변수, 정리 주기, gitignore
